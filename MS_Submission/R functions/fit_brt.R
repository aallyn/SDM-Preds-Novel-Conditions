...
aja.roc<- function (obsdat, preddat) {
...
aja.calibration <- function(obs, preds, family = "binomial")  {
#
  ...
  ...)                                      # allows for any additional plotting parameters
{
    ...
    if (FALSE) {
        # data = readRDS(here::here("data/sim_spp/train_test_lme/cc/MHW12_1985-01-01_to_2011-01-01.rds"))[[1]]
        # predictors_vec = c("depth", "oisst_daily")
        # response = "Real"
        # family = "bernoulli"
        # tree_complexity = 3
        # learning_rate = 0.01
...
# includes estimates of the standard error of the fitted values added 2nd may 2005
	...
	for (i in 1:n.folds) {
		...
		if (family == "bernoulli") {
			cv.roc.stats[i] <- aja.roc(y_i,u_i)
			cv.calibration.stats[i,] <- aja.calibration(y_i,u_i,"binomial")
			threshold.stats[i] <- approx(ppoints(u_i), sort(u_i,decreasing = T), prevalence)$y
		}

		if (family == "poisson") {
			cv.calibration.stats[i,] <- aja.calibration(y_i,u_i,"poisson")
		}
	}
...
#extract fitted values and summary table
	...
	if (family == "bernoulli") {  # do this manually as we need the residuals
		...
		self.roc <- aja.roc(y_i,u_i)
		self.calibration <- aja.calibration(y_i,u_i,"binomial")
	}
	...
	if (family == "poisson") {   # do this manually as we need the residuals
		...
		residuals <- sqrt(abs(deviance.contribs * 2))
		residuals <- ifelse((y_i - u_i) < 0, 0 - residuals, residuals)
		self.calibration <- aja.calibration(y_i,u_i,"poisson")
	}
...
#' @title Fit boosted regression tree model to training presence/absence dataset
#'
#' @description This function will fit a boosted regression tree model to training presence/absence dataset
#'
#' @param train_data = A dataframe with the training data
#' @param predictors_vec = A sequence of character strings with the column names of train_data to use as the BRT predictor variables. Each predictor variable character string should be separated by a comma.
#' @param response = A character vector with the column name of train_data to use as the BRT response variable
#' @param family = A character vector with the response family to use in fitting the BRT. 
#' @param tree_complexity = An integer value for the complexity of the interactions that the model will fit.
#' @param learning_rate = A decimal value determining...
#' @param bag_fraction = A decimal value indicating the amount of the training data to be used in each model fit iteration.
#'
#' @return A fitted boosted regression tree model object.
#'
#' @export

fit_brt<- function(train_data, predictors_vec, response, family, tree_complexity, learning_rate, bag_fraction){
  
  # For debugging
  if(FALSE){
    train_data = readRDS(here::here("data/sim_spp/train_test_lme/cc/MHW12_1985-01-01_to_2011-01-01.rds"))[[1]]
    predictors_vec = c("depth", "oisst_daily")
    response = "Real"
    family = "bernoulli"
    tree_complexity = 3
  ...
  # Predictors vector formatting
  predictors_vec_use<- unlist(as.vector(strsplit(predictors_vec, split = ",")))
  
  # Some preliminary checks
  # Check that all predictors are in training data
  if(!all(predictors_vec_use %in% colnames(train_data))){
      stop(print("Check predictor variables and training data. Some predictors are missing from the training data."))
  }

  # Fit the BRT model
  # Get columns
  brt_fit <- gbm.step.aja(
          data = train_data, 
          gbm.x = 6:7, 
          gbm.y = 4, 
          family = family, 
...
#' @title Main fit boosted regression tree model to training presence/absence dataset
#'
#' @description This function reads in a training dataset from a training_testing list and then fits a BRT model using the 'fit_brt' function with supplied arguments for the predictors, response, family, tree complexity, learning rate and bag fraction. The function then saves this fitted BRT model as an .rds object according to the file name and path provided.  
#' 
#' @param training_testing_list_path = A path to a list with the training and testing data slices
#' @param predictors = A sequence of character strings with the column names of train_data to use as the BRT predictor variables. Each predictor variable character string should be separated by a comma.
#' @param response = A character vector with the column name of train_data to use as the BRT response variable
#' @param family = A character vector with the response family to use in fitting the BRT. 
#' @param tree_complexity = An integer value for the complexity of the interactions that the model will fit.
#' @param learning_rate = A decimal value determining...
#' @param bag_fraction = A decimal value indicating the amount of the training data to be used in each model fit iteration.
#' @param out_file = A file path and name to save the fitted BRT as a .rds file.

#' @return Saves a list with sampled presence/absences.
#'
#' @export

main_fit_brt<- function(training_testing_list_path, predictors_vec, response, family, tree_complexity, learning_rate, bag_fraction, out_file){
  
  # For debugging
  if(FALSE){
  }
  
  # Load the list with training and testing slices
  train_test_data<- readRDS(training_testing_list_path)

  # Get just the training data
  train_data<- train_test_data[[1]]

  brt_fit_out<- fit_brt(train_data = train_data, predictors_vec = predictors_vec, response = response, family = family, tree_complexity = tree_complexity, learning_rate = learning_rate, bag_fraction = bag_fraction)

  # Save it
  saveRDS(brt_fit_out, file = out_file)
}
